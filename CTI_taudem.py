...
#The version of TAUDEM used in this code is : V5.1.3
#A python library was considered as an alternative to TAUDEM but abandonned because of it's early stage : Pygeoprocessing
...
def processing() :
	...
	# Mask the glaciers as proposed as well as the seams between the two DEM 
	mask_rasters(os.path.join(out,'AKPCTR_DEMfel.tif'), [ glacier , seam ],1,os.path.join(out,'AKPCTR_NoIceSeam.tif'),nodata_value)

	#Run the flow direction algoritm from TAUDEM
	...
	#Run the contributing area algorithm from TAUDEM
	...
	#Run the flow direction algorithm from TAUDEM
...
if __name__ == '__main__':
	...
	#Base DEM from the whole area. Canada and Alaska side merged together
	base_DEM = '/workspace/Shared/Users/jschroder/CTI/data/CTI_ref/AKPCTR_DEMcor.tif' 

	#Set the location of the Glaciers area as well as the seam, both used for masking the DEM
